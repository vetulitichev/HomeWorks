...
function reverseEachWord(arg,flag) {
        arg = arg.split(' ');
        for (let i = 0; i < arg.length; i++) {
            arg[i] = arg[i].split('');
            arg[i].reverse();
...
let func = str => str.split(' ').reduce((obj,item) => obj[name]? obj[name]+=1 : obj[name] =0,0)

//if(obj[item]){obj[item]+=1} else {obj[item] = 0;}
...
//uniqueElements(notUniqArray); //1,2,5,10,25,30,0,22,3,
//uniqueElements([1,1,2,3]); // 1,2,3
